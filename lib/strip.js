...
//
// Separately, a whole line matching a known AI banner/footer format (e.g.
// "🤖 Generated with [Claude Code](...)") is also stripped — these are plain
// body lines some tools insert, not Co-Authored-By trailers, so they need a
// full-line match of their own (lib/patterns.js AI_BANNER_LINES). Only exact,
// confirmed formats are listed there to avoid stripping ordinary body text
// that happens to mention a tool by name.
//
...
function stripMessage(message, opts) {
  ...
  var baseBanners = opts.banners && opts.banners.length ? opts.banners : patterns.AI_BANNER_LINES || []
  ...
  var banners = baseBanners.concat((opts.extraBanners || []).map(escapeRe))
  ...
  // Standalone AI banner/footer lines (e.g. "🤖 Generated with [Claude
  // Code](...)") — not a trailer, so matched as a whole line on its own,
  // independent of the Co-Authored-By regex above. Only built when there is
  // at least one banner pattern: an empty alternation would compile to `()`,
  // which matches every line.
  var bannerRe = banners.length ? new RegExp('^\\s*(?:' + banners.join('|') + ')\\s*$', 'i') : null
  ...
  for (var i = 0; i < lines.length; i++) {
    var line = lines[i]
    if (line.length <= MAX_TRAILER_LINE_LENGTH && (re.test(line) || (bannerRe && bannerRe.test(line)))) continue
    if (line.trim() === '') {
