...
function toPosixEre(s) {
  ...
  return String(s)
    ...
    // POSIX ERE has no non-capturing group syntax — `(?:` is not a group
    // modifier there, it's `(` followed by a literal `?` with nothing to
    // quantify, which is undefined/broken behavior in most awk regex
    // engines. ERE groups are already capture-less as far as this tool cares
    // (nothing here reads back-references), so `(?:` safely becomes `(`.
    .replace(/\(\?:/g, '(')
    .replace(/\\w/g, '[[:alnum:]_]')
    .replace(/\\W/g, '[^[:alnum:]_]')
    .replace(/\\d/g, '[[:digit:]]')
...
function buildHook() {
  ...
  var banners = (patterns.AI_BANNER_LINES || []).map(toPosixEre)

  var nameAlt = names.join('|')
  var emailAlt = emails.join('|')
  var domainAlt = domains.join('|')
  var bannerAlt = banners.join('|')
  ...
  // Code](...)") — not a trailer, so matched as a whole line on its own via a
  // separate ^...$-anchored pattern, independent of PAT above.
  var bannerPat = '^[[:space:]]*(' + bannerAlt + ')[[:space:]]*$'
