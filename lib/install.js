...
function installAt(hooksDir, hookBody) {
  ...
  if (!fs.existsSync(origPath)) {
    ...
    // An orig backup already exists (previous install); keep it. The current
    // hookPath is foreign again — overwrite with our wrapper below.
  }
  writeExecutable(ncPath, hookBody)
  writeExecutable(hookPath, wrapperScript())
  return 'wrapped'
...
function installLocal(noNode) {
  ...
  if (localHooksPath) {
    ...
    warn('using local core.hooksPath: ' + localHooksPath)
  } else {
    ...
    if (globalHp) {
      warn(
        'global core.hooksPath is set to ' +
          globalHp +
          '; a per-project hook in .git/hooks will NOT run unless you set a local core.hooksPath. Consider `no-coauthor install --global`.'
      )
    }
  }

  var result = installAt(hooksDir, hookBody)
  warn('hook ' + result + ' at ' + path.join(hooksDir, 'commit-msg'))
...
function installGlobal(noNode) {
  ...
  if (existingGlobal) {
    ...
    warn('using existing global core.hooksPath: ' + existingGlobal)
  } else {
    globalDir = path.join(os.homedir(), '.git-hooks')
    execSync('git config --global core.hooksPath ' + JSON.stringify(globalDir))
  }

  var result = installAt(globalDir, hookBody)
  warn('global hook ' + result + ' at ' + path.join(globalDir, 'commit-msg'))
  warn('all git repos on this machine will strip AI co-author trailers')
  warn('repos with a local core.hooksPath are not affected by the global hook')
}
