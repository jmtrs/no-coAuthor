...
// `pnpm`/`npm install` (its `prepare` script). Anything no-coauthor writes
// into .husky/_ is wiped on the next install, so an install there is silently
// non-durable. The user-authored .husky/<hook> files one level up are stable
// (husky never overwrites those). Detect the volatile dir by path (.husky/_
// under the worktree root) or by husky's marker file inside it (older v8).
function isHuskyVolatileHooksDir(root, hooksDir) {
  var rel = path.relative(root, hooksDir)
  if (rel === path.join('.husky', '_')) return true
  try {
    if (fs.statSync(path.join(root, '.husky', '_', 'husky.sh')).isFile()) return true
  } catch (e) {}
  return false
}
...
function installLocal(noNode) {
  ...
  if (localHooksPath) {
    ...
    if (isHuskyVolatileHooksDir(root, hooksDir)) {
      ui.err(
        'core.hooksPath is ' +
          localHooksPath +
          " — that is husky's generated hooks dir, which husky rewrites on every install/prepare, so a hook written there is NOT durable (it will be wiped the next time you run pnpm/npm install)." +
          ' Chain no-coauthor from your user-authored hook instead: edit .husky/commit-msg to add, after your existing commit-msg command:' +
          '\n    "$HOME/.git-hooks/commit-msg" "$1"' +
          '\n  (install that global hook once with: no-coauthor install --global)' +
          '\nRefusing to install a hook that would not survive the next install.'
      )
      process.exit(1)
    }
  } else {
