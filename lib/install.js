...
// Node's module-type resolution for an extensionless script walks up from
// the script to the nearest package.json. `.git/hooks` (or a global
// ~/.git-hooks) normally has none, so it lands on the TARGET REPO's own
// package.json — if that declares "type": "module" (Vite, and increasingly
// any ESM-first project), Node loads our CommonJS hook body as ESM and
// `require` is undefined, crashing every commit. Dropping a minimal
// package.json right in the hooks dir stops the walk there, pinning the
// hook to CommonJS regardless of what the real project declares. Only
// needed for the Node hook — the POSIX one has no module system to confuse.
var COMMONJS_PACKAGE_JSON = '{"type":"commonjs"}\n'

function ensureCommonJsPackageJson(hooksDir) {
  var p = path.join(hooksDir, 'package.json')
  if (!fs.existsSync(p)) {
    fs.writeFileSync(p, COMMONJS_PACKAGE_JSON)
  }
}

function installAt(hooksDir, hookBody, noNode) {
  fs.mkdirSync(hooksDir, { recursive: true })
  if (!noNode) ensureCommonJsPackageJson(hooksDir)
...
function installLocal(noNode) {
  ...
  var result = installAt(hooksDir, hookBody, noNode)
...
function installGlobal(noNode) {
  ...
  var result = installAt(globalDir, hookBody, noNode)
...
module.exports.COMMONJS_PACKAGE_JSON = COMMONJS_PACKAGE_JSON
