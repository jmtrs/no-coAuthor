...
var AI_BANNER_LINES = [
  // Claude Code's default commit-body attribution line. Confirmed variants
  // across CLI versions/docs (no single canonical string):
  //   🤖 Generated with [Claude Code](https://claude.com/claude-code)   (markdown link)
  //   Generated with [Claude Code](https://claude.ai/code)              (legacy link, markdown)
  //   🤖 Generated with Claude Code                                     (no link)
  //   Generated with Claude Code (https://claude.com/claude-code)       (plain parenthetical link)
  // Emoji prefix is optional throughout. Note this is fully overridable by
  // the user/team via Claude Code's own `attribution.commit` setting — a
  // custom string there won't match any fixed pattern here; add it via this
  // tool's own `banners` config instead (see README Configuration).
  '(🤖\\s*)?Generated with (\\[Claude Code\\]\\(https://claude\\.(ai/code|com/claude-code)\\)' +
    '|Claude Code(\\s+\\(https://claude\\.(ai/code|com/claude-code)\\))?)'
]
