...
// Record/unit separator control chars (not NUL — process arguments can't
// contain a null byte, since argv is C strings under the hood; execFileSync
// throws "must be a string without null bytes" if you try). \x1e/\x1f are
// the ASCII characters designed for exactly this "won't appear in real text"
// delimiter role.
var SEP = '\x1e'
var FIELD_SEP = '\x1f'
...
function readConfig() {
  ...
  var candidates = [path.join(process.cwd(), '.no-coauthorrc.json'), path.join(os.homedir(), '.no-coauthorrc.json')]
...
function commitsInRange(range) {
  var out
  try {
    out = execFileSync('git', ['log', '--format=%H' + FIELD_SEP + '%B' + SEP, range], { encoding: 'utf8' })
  } catch (e) {
