...
function commitsInRange(range) {
  ...
  try {
    ...
    ui.err('check: failed to read git log for range "' + range + '"')
    console.error(ui.pc.red((e.stderr || e.message || String(e)).toString().trim()))
    process.exit(2)
  }
  return out
    .split(SEP)
...
module.exports = function check(range) {
  ...
  if (commits.length === 0) {
    ui.line('check: no commits in range "' + range + '"')
    process.exit(0)
  }
  ...
  if (offenders.length === 0) {
    ui.say(true, commits.length + ' commit(s) in range "' + range + '" — no AI co-author trailers found')
    process.exit(0)
  }

  ui.say(false, 'found AI co-author trailers in ' + offenders.length + ' commit(s):')
  offenders.forEach(function (c) {
    console.error('')
    console.error('  ' + ui.pc.dim(c.sha.slice(0, 12)))
    c.message.split('\n').forEach(function (line) {
      if (/Co-Authored-By/i.test(line)) console.error('    ' + ui.pc.red(line.trim()))
    })
  })
  console.error('')
  ui.err('reject this PR, or amend/rebase to remove the trailers before merging')
  process.exit(1)
}
